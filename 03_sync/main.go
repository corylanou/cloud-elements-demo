...
func main() {
	...
	googleFiles, err := listAndCreateFolder(path, cloudElements.GOOGLE_DRIVE, folders)
	if err != nil {
		log.Fatalf("failed to create/list folder %s - %s:  %+v", path, cloudElements.GOOGLE_DRIVE, err)
	}

	dropboxFiles, err := listAndCreateFolder(path, cloudElements.DROPBOX, folders)
	if err != nil {
	...
	for _, s := range sync {
		...
		r, err := folders.GetFileById(s.file.Id, s.source)
		if err != nil {
			log.Fatalf("\nfailed to retrieve file %s from %s:  %+v", s.file.Name, s.source, err)
		}
		_, err = folders.CreateFile(s.file, false, r, s.destination)
		if err != nil {
...
func listAndCreateFolder(path string, provider cloudElements.Provider, folders *cloudElements.Folders) (cloudElements.Files, *cloudElements.Error) {
	files, err := folders.Contents(path, provider)
	if err != nil {
		if err.StatusCode == http.StatusNotFound {
			// The folder doesn't exist, so we need to create it
			_, err := folders.CreateFolder(path, cloudElements.DROPBOX)
			if err != nil {
